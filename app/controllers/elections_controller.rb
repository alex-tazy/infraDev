class ElectionsController < ApplicationController
  ...
  # POST /elections.json
  def create
    choices_names = params[:election]['choices']
    @election = Election.new(election_params)
    
    if @election.save
      choices_names.each do |name|
        Choice.new(name: name, election_id: @election.id).save
      end
    end
  ...
  private
    ...
    def election_params
      params.require(:election).permit(:start_date, :end_date, :groups, :title, :choices)
    end
end
